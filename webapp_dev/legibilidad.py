...
def szigriszt_pazos(text):
    ...
    '''
    return round(206.835 - 62.3 * (count_all_syllables(numbers2words(text)) / count_words(numbers2words(text))) - (
                count_words(numbers2words(text)) / count_sentences(text)), 2)
...
def gutierrez(text):
    ...
    '''
    legibguti = 95.2 - 9.7 * (count_letters(text) / count_words(text)) - 0.35 * (
                count_words(text) / count_sentences(text))
